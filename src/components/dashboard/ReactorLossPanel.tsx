  ...
  return (
    <section className="panel flex h-full w-[420px] shrink-0 flex-col gap-4 overflow-hidden p-5">
      <div className="flex min-w-0 items-center justify-between gap-3">
        ...
        <p className="shrink-0 truncate text-[11px] text-process-muted">데모 5초 = 공정 1분 · {playbackMinute}/{maxPlaybackMinute}분</p>
      </div>
      <div className="flex min-w-0 flex-col gap-2.5 overflow-hidden">
        {reactors.map((reactor) => {
          const style = severityStyle[reactor.status];
          const unmitigatedWidth = maxLoss === 0 ? 0 : (reactor.unmitigatedLossKwh / maxLoss) * 100;
          const avoidableWidth = maxLoss === 0 ? 0 : (reactor.avoidableLossKwh / maxLoss) * 100;
            ...
            <div key={reactor.id} className="flex min-w-0 items-center justify-between gap-3 rounded-[10px] border border-process-line bg-process-bg px-3 py-2">
              ...
              <div className="flex min-w-[182px] shrink-0 flex-col gap-1">
                <div className="flex items-center gap-2">
                  <p className="w-[88px] whitespace-nowrap text-right text-[11px] font-bold text-process-red">방치 {reactor.unmitigatedLossKwh.toFixed(2)}</p>
                  <div className="h-1.5 min-w-0 flex-1 overflow-hidden rounded bg-process-line">
                    <div className="h-full rounded bg-process-red" style={{ width: `${unmitigatedWidth}%` }} />
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <p className="w-[88px] whitespace-nowrap text-right text-[11px] font-bold text-process-cyan">예방 {reactor.avoidableLossKwh.toFixed(2)}</p>
                  <div className="h-1.5 min-w-0 flex-1 overflow-hidden rounded bg-process-line">
