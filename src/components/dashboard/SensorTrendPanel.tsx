...
type SensorTrendPanelProps = {
  ...
  onSelectReactor: (reactorId: string) => void;
};
...
function SensorTrendPanelComponent({ summary, dataUpdatedAt, trend, selectedReactorId, onSelectReactor }: SensorTrendPanelProps) {
  return (
    <section className="panel flex min-w-0 flex-1 flex-col gap-5 overflow-hidden p-6">
      <div className="flex min-w-0 items-center justify-between gap-4">
        <div className="flex min-w-0 flex-col gap-1">
          ...
          <p className="truncate text-xs text-process-muted">{trend?.reactorId ?? selectedReactorId} 고장 전후 온도·압력·유량·진동·전류 추이 모니터링</p>
        </div>
      ...
      <div className="flex min-w-0 flex-wrap items-center gap-3">
        <div className="flex min-w-0 items-center gap-2 rounded-[10px] border border-process-line bg-process-bg px-2.5 py-2">
          ...
          <div className="flex min-w-0 flex-wrap gap-1.5">
            {REACTOR_IDS.map((id) => (
              <StatusChip key={id} active={id === selectedReactorId} onClick={() => onSelectReactor(id)}>
                {id}
              </StatusChip>
            ))}
          </div>
