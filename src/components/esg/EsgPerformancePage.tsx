  ...
  return (
    <header className="flex w-full items-center justify-between gap-8">
      ...
      <div className="flex shrink-0 items-center gap-4">
        ...
        >
          <svg aria-hidden="true" className="size-4" fill="none" viewBox="0 0 18 18">
...
function MonthlySavingsChart({
  ...
  const hoveredX = hoveredIndex === null ? 0 : pointX(hoveredIndex, rows.length, chart.width);
  const hoveredDailyY = hoveredRow ? scaleY(hoveredRow.monthly, chart.max, chart.height) : 0;
  const hoveredCumulativeY = hoveredRow ? scaleY(hoveredRow.cumulative, chart.max, chart.height) : 0;
  ...
  const handleChartPointerMove = (event: ReactMouseEvent<SVGRectElement>) => {
    ...
    const index = rows.length <= 1
      ? 0
      : Math.round((x / chart.width) * (rows.length - 1));
  ...
  return (
    <section className="flex h-full min-h-[520px] min-w-0 flex-col gap-5 rounded-card border border-[#20273d] bg-[#121626] p-6">
      <div className="flex items-center justify-between gap-6">
        ...
        <div className="flex shrink-0 translate-y-3 flex-col items-end gap-4">
          <div className="flex items-baseline gap-2">
            <span className="text-[10px] font-bold text-[#626e8a]">최고 일일 절감량</span>
            <span className="flex items-baseline gap-1.5">
              <span className="text-[11px] font-bold text-[#8f9bb3]">{bestMonth}월 {bestDay}일</span>
              <strong className="text-[16px] font-extrabold text-white">{chart.best.row.monthly.toLocaleString("ko-KR", { maximumFractionDigits: 2 })}</strong>
              <span className="text-[10px] font-bold text-process-green">kWh</span>
            </span>
          </div>
          <div className="flex items-center gap-4 text-xs text-[#8f9bb3]">
            <span className="flex items-center gap-1.5"><span className="size-2.5 rounded-sm bg-process-green" />일일 절감량</span>
      ...
      <div className="flex h-[330px] min-w-0 gap-2">
        ...
        <svg viewBox={`0 0 ${chart.width} ${chart.height}`} className="h-[330px] min-w-0 flex-1 overflow-visible">
          {[0, 1, 2, 3, 4].map((index) => {
            const y = 10 + (index / 4) * (chart.height - 20);
            return <line key={index} x1="0" x2={chart.width} y1={y} y2={y} stroke="#20273d" strokeWidth="1" />;
          })}
          {rows.map((row, index) => {
          ...
          {hoveredRow ? (
            <g pointerEvents="none">
              <line x1={hoveredX} x2={hoveredX} y1="0" y2={chart.height} stroke="#8f9bb3" strokeDasharray="4 4" opacity="0.65" />
              <circle cx={hoveredX} cy={hoveredDailyY} r="5" fill="#10b981" stroke="#121626" strokeWidth="2" />
              <circle cx={hoveredX} cy={hoveredCumulativeY} r="4" fill="#f59e0b" stroke="#121626" strokeWidth="2" />
              <foreignObject
                height="72"
                width="174"
                x={Math.min(Math.max(hoveredX - 87, 0), chart.width - 174)}
                y={Math.min(Math.max(Math.min(hoveredDailyY, hoveredCumulativeY) - 78, 0), chart.height - 72)}
              >
                <div className="rounded-lg border border-[#34405c] bg-[#090b1a]/95 px-3 py-2 text-[10px] shadow-[0_12px_28px_rgba(0,0,0,0.42)]">
                  <p className="mb-1 font-extrabold text-white">{hoveredRow.month}</p>
                  <p className="flex justify-between gap-3 text-process-green"><span>일일 절감량</span><strong>{hoveredRow.monthly.toLocaleString("ko-KR", { maximumFractionDigits: 2 })} kWh</strong></p>
                  <p className="mt-0.5 flex justify-between gap-3 text-[#f59e0b]"><span>누적 절감량</span><strong>{hoveredRow.cumulative.toLocaleString("ko-KR", { maximumFractionDigits: 2 })} kWh</strong></p>
                </div>
              </foreignObject>
            </g>
          ) : null}
      ...
      <div className="flex w-full justify-between pl-10 text-[11px] text-[#8f9bb3]">
        {rows
          .filter((_, index) => index === 0 || index === rows.length - 1 || (index + 1) % 5 === 0)
          .map((row) => <span key={row.month}>{row.month}</span>)}
      </div>
    </section>
  );
}
...
function QuarterTargetManager({
  ...
  const quarterPeriods: Record<string, string> = { Q1: "1~3월", Q2: "4~6월", Q3: "7~9월", Q4: "10~12월" };
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-[#050711]/75 p-6 backdrop-blur-sm" role="presentation" onMouseDown={onClose}>
      <section
        ...
        <header className="flex items-center justify-between border-b border-[#20273d] px-6 py-5">
          <div className="flex items-center gap-3">
            <span className="flex size-10 items-center justify-center rounded-xl bg-process-cyan/10 text-process-cyan">
              <svg aria-hidden="true" className="size-5" fill="none" viewBox="0 0 18 18">
                <circle cx="9" cy="9" r="6.5" stroke="currentColor" strokeWidth="1.4" />
                <circle cx="9" cy="9" r="3.2" stroke="currentColor" strokeWidth="1.4" />
                <circle cx="9" cy="9" r="1" fill="currentColor" />
              </svg>
            ...
            <div>
              <h2 className="text-[17px] font-extrabold text-white" id="quarter-target-title">분기 에너지 절감 목표 관리</h2>
              <p className="mt-1 text-[11px] font-medium text-[#8f9bb3]">분기별 목표 전력량을 입력하면 달성률이 자동으로 계산됩니다.</p>
            </div>
          </div>
          <button aria-label="닫기" className="flex size-8 items-center justify-center rounded-lg text-[#8f9bb3] transition hover:bg-[#20273d] hover:text-white" type="button" onClick={onClose}>
            <svg aria-hidden="true" className="size-4" fill="none" viewBox="0 0 16 16"><path d="m4 4 8 8m0-8-8 8" stroke="currentColor" strokeLinecap="round" strokeWidth="1.5" /></svg>
          </button>
        </header>
        <div className="grid grid-cols-2 gap-3 p-6">
          {Object.keys(DEFAULT_QUARTER_TARGETS).map((quarter) => (
            <label className="rounded-xl border border-[#20273d] bg-[#0d1020] p-4 transition focus-within:border-process-cyan/50" key={quarter}>
              <span className="mb-3 flex items-center justify-between">
                <strong className="text-[14px] text-white">{quarter}</strong>
                <span className="text-[10px] font-semibold text-[#626e8a]">{quarterPeriods[quarter]}</span>
              </span>
              <span className="flex h-11 items-center rounded-lg border border-[#29324a] bg-[#090b1a] px-3 focus-within:border-process-cyan/60">
                <input
                  ...
                  onChange={(event) => {
                    ...
                    if (/^\d*\.?\d*$/.test(value)) setDrafts((current) => ({ ...current, [quarter]: value }));
                  }}
                />
                <span className="text-[11px] font-bold text-[#8f9bb3]">kWh</span>
              </span>
            </label>
          ))}
        </div>
        <footer className="flex items-center justify-between gap-3 border-t border-[#20273d] px-6 py-4">
          <p className="text-[11px] font-semibold text-[#ef6a75]">{saveError}</p>
          <div className="flex shrink-0 items-center gap-2">
            <button className="h-10 rounded-lg border border-[#29324a] px-4 text-[12px] font-bold text-[#8f9bb3] transition hover:bg-[#20273d] hover:text-white disabled:opacity-50" disabled={isSaving} type="button" onClick={onClose}>취소</button>
            <button className="h-10 rounded-lg bg-process-cyan px-5 text-[12px] font-extrabold text-[#090b1a] transition hover:bg-[#6ff6ff] disabled:cursor-wait disabled:opacity-60" disabled={isSaving} type="button" onClick={() => void onSave(drafts)}>{isSaving ? "저장 중..." : "목표 저장"}</button>
          </div>
        </footer>
      </section>
    </div>
...
function QuarterlyMilestones({
  ...
  const hasSelectedQuarterTarget = Number.isFinite(selectedQuarterTarget) && selectedQuarterTarget > 0;
  ...
  return (
    <section className="flex flex-col gap-3.5 rounded-card border border-[#20273d] bg-[#121626] p-5">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-[15px] font-bold text-white">분기 에너지 절감 목표 달성 현황</p>
          <p className="mt-1 text-[10px] font-semibold text-[#626e8a]">분기의 첫 달에 설정한 목표 대비 누적 절감 달성률</p>
        </div>
        <span className={`mr-6 shrink-0 rounded-lg border px-3 py-2 text-[11px] font-bold ${hasSelectedQuarterTarget ? "border-process-cyan/25 bg-process-cyan/5 text-process-cyan" : "border-[#29324a] bg-[#0d1020] text-[#626e8a]"}`}>
          {hasSelectedQuarterTarget ? `${selectedQuarterLabel} 목표 ${selectedQuarterTarget.toLocaleString("ko-KR")} kWh` : "목표 미설정"}
        </span>
      </div>
      <div className="flex flex-col gap-2.5">
        {(selectedQuarter ? [selectedQuarter] : []).map((quarter) => {
          ...
          const rawPercent = hasTarget ? Math.round((quarter.totalKwh / targetKwh) * 100) : 0;
          const percent = Math.min(rawPercent, 100);
          const isAchieved = hasTarget && rawPercent >= 100;
          const color = isAchieved ? "#10b981" : "#f59e0b";
          return (
            <div key={quarter.label} className="flex min-h-[64px] items-center rounded-lg border border-[#20273d] bg-[#0d1020] px-4 py-3">
              <div className="flex w-full items-center gap-4">
                <p className={`w-[88px] shrink-0 whitespace-nowrap text-[12px] font-bold ${isAchieved ? "text-process-green" : "text-[#f59e0b]"}`}>
                  {quarter.label} <span className="font-semibold text-[#626e8a]">({quarter.period})</span>
                </p>
                ...
                <span className={`w-11 rounded px-1.5 py-0.5 text-center text-[10px] font-bold ${isAchieved ? "bg-process-green/10 text-process-green" : "bg-[#f59e0b]/15 text-[#f59e0b]"}`}>
                  {percent}%
                </span>
                <p className={`w-[104px] shrink-0 text-right text-[11px] font-bold ${quarter.totalKwh > 0 ? "text-[#f59e0b]" : "text-[#8f9bb3]"}`}>누적 {quarter.note}</p>
              </div>
            </div>
          );
        })}
      ...
      <div className="flex items-center justify-center rounded-lg border border-process-cyan/20 bg-process-cyan/5 p-3">
        <div className="flex min-w-0 items-center justify-center gap-6 text-center text-xs font-semibold">
          <p className="whitespace-nowrap text-process-cyan">
            {displayYearMonth(selectedMonth)} 절감량 = <strong className="ml-1 text-white">{selectedMonthSavingKwh.toLocaleString("ko-KR", { maximumFractionDigits: 2 })} kWh</strong>
          </p>
          ...
          <div className="relative flex items-center whitespace-nowrap text-process-cyan">
            ...
            = <strong className="ml-1 text-white">{selectedQuarter?.totalKwh.toLocaleString("ko-KR", { maximumFractionDigits: 2 }) ?? "0"} kWh</strong>
          </div>
        </div>
      </div>
    </section>
...
function EsgPageContent({
  ...
  const [quarterTargets, setQuarterTargets] = useState<Record<string, string>>(DEFAULT_QUARTER_TARGETS);
  ...
  async function handleSaveQuarterTargets(targets: Record<string, string>) {
    ...
    try {
      ...
      setTargetSaveError("DB에 저장하지 못했습니다. 백엔드 연결을 확인해 주세요.");
    } finally {
